...
class PDeposito extends Geleia {
    ...
    function QuantidadeLeituraPorCentro($Centro) {
        global $db;

        $sql = " SELECT SUM(etiq_quantidade) as qnt_etiqueta FROM etiquetas WHERE etiq_depo_centro = (SELECT depo_id FROM deposito WHERE depo_centro = {$Centro}) ";
