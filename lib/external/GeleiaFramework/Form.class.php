...
abstract class PForm extends UserControl {
    ...
    function GetById($ReturnArray = false) {
        ...
        if ($this > SQL_GetById != null) {
            if ($rset = $db->ExecSQL($this->SQL_GetById)) {
                //echo $this->SQL_GetById;
                if ($ReturnArray) {
                    return $db->GetArray($rset);
                }
                return $db->GetObject($rset);
            }
        } else {
    ...
    function Delete() {
        ...
        if ($this->SQL_Delete != null) {
            if ($rset = $db->ExecSQL($this->SQL_Delete)) {
...
// 			die();
    ...
    function GetInfoAboutTable($Table) {
        ...
        $sql = "SELECT * FROM information_schema.`COLUMNS` WHERE TABLE_NAME='" . ($Table) . "' AND TABLE_SCHEMA='" . $db->db . "' ORDER BY ORDINAL_POSITION ASC";
        //die($sql);
        if ($rset = $db->ExecSQL($sql)) {
            ...
            if ($FieldPreferences->{'default-value-sql'} != "") {
                ...
                if ($ObjTable->COLUMN_NAME == $this->RemovedFieldName) {
                    ...
                    if ($filename = $this->UploadSingleFile($File, $FieldPreferences)) {
                        ...
                        $SqlTemp .= "NULL,";/** CAN OCCUR SOME PROBLEM WHEN ANOTHER FIELD COMES AFTERWARDS * */
                    }
